...
def seed_tasks():
    ...
    ai_task1= Task(name="Conduct quantitative data analysis", description="Narrow down a specific, relevant audience to identify the users who conduct the highest percentages of multi-studies.", end_date=datetime(2022,7,21), status_id=2, priority_id=2, assigner_id=2, assignee_id=1, project_id=5, is_completed=True)
    ai_task2= Task(name="Conduct qualitiative interviews", description="Expand beyond the quantitative data, ask questions about why and how to support user research", end_date=datetime(2022,7,26), status_id=1, priority_id=3, assigner_id=5, assignee_id=3, project_id=5, is_completed=True)
    ai_task3= Task(name="Construct product strategy with metrics", description="Present strongest ideas to stakeholders, using Desirability, Feasibility, Viability metrics", end_date=datetime(2022,8,4), status_id=2, priority_id=1, assigner_id=5, assignee_id=7, project_id=5, is_completed=False)
    ai_task4= Task(name="Build a usable dataset ", description="Work with engineers to pull data from API queries to parse JSON files into relational database and readable data", end_date=datetime(2022,8,16), status_id=3, priority_id=1, assigner_id=3, assignee_id=2, project_id=5, is_completed=False)
    ai_task5= Task(name="Transform dataset into pattern set", description="Transform textual values into numerical vectors into readable outputs or predictions from data models", end_date=datetime(2022,8,21), status_id=2, priority_id=3, assigner_id=8, assignee_id=1, project_id=5, is_completed=False)
    ai_task6= Task(name="Seasonality analysis", description="Seasonality can affect the data so we must normalize the data to eliminate biases", end_date=datetime(2022,8,25), status_id=3, priority_id=3, assigner_id=1, assignee_id=1, project_id=5, is_completed=False)
    ai_task7= Task(name="Conduct Stationarity Test", description="Data can also be affected by time, run autocorrelation and statistical tests to determine if data is stationary", end_date=datetime(2022,9,3), status_id=1, priority_id=2, assigner_id=1, assignee_id=8, project_id=5, is_completed=False)
    ai_task8= Task(name="Understand association patterns", description="Cluster data into certain pattern sets and understand user behavior", end_date=datetime(2022,9,8), status_id=2, priority_id=1, assigner_id=2, assignee_id=3, project_id=5, is_completed=False)
    ai_task9= Task(name="Process Mining Analysis", description="Where do users experience the most complex behaviors in the product? What can we do to ease their experience and cut down on attrition?", end_date=datetime(2022,9,15), status_id=3, priority_id=1, assigner_id=5, assignee_id=7, project_id=5, is_completed=False)
    ai_task10= Task(name="Prepare proposition slide deck", description="Create clean slides to present ideas to stakeholders, submit for review", end_date=datetime(2022,9,22), status_id=2, priority_id=3, assigner_id=1, assignee_id=7, project_id=5, is_completed=False)

    persona_task1= Task(name="Conduct interviews", description="Conduct interviews with Product and Engineering Teams, as well as the Company's users", end_date=datetime(2022,7,18), status_id=1,priority_id=1, assigner_id=1, assignee_id=6, project_id=6, is_completed=True)
    persona_task2= Task(name="Formulate thesis from interview data", description="Construct the Why the Company needs formalized personas, what makes personas valuable, and what is needed to ensure adoption. Also, understand any empathy gaps and questions we still have about our users", end_date=datetime(2022,7,24), status_id=2,priority_id=3, assigner_id=3, assignee_id=3, project_id=6, is_completed=True)
    persona_task3= Task(name="Data visualization", description="Break raw research out into a visual map of themes and common feelings and thoughts from user interviews", end_date=datetime(2022,7,29), status_id=1,priority_id=2, assigner_id=2, assignee_id=1, project_id=6, is_completed=False)
    persona_task4= Task(name="Propose product team strategy", description="Given data from conducted interviews, propose plan as to how we can explore persona visualizations and frameworks to entice various stakeholders", end_date=datetime(2022,8,5), status_id=2,priority_id=1, assigner_id=6, assignee_id=7, project_id=6, is_completed=False)
    persona_task5= Task(name="Construct product strategy", description="How does our research and thesis align with the current product roadmap?", end_date=datetime(2022,8,14), status_id=3,priority_id=1, assigner_id=7, assignee_id=2, project_id=6, is_completed=False)
    persona_task6= Task(name="Reflect on future customers", description="Understand how our current strategy will affect current customers but also future customers with changing landscapes", end_date=datetime(2022,8,29), status_id=1,priority_id=3, assigner_id=1, assignee_id=7, project_id=6, is_completed=False)
